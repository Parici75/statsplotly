...
class TestColorSpecifier:
    ...
    def test_shared_coloraxis(self):
        coloraxis = ColorSpecifier.build_from_color_data(
            color_data=EXAMPLE_VALID_INT_COLOR_DATA
        ).build_coloraxis(color_data=EXAMPLE_VALID_INT_COLOR_DATA, shared=True)
        assert coloraxis.cmin == EXAMPLE_VALID_INT_COLOR_DATA.min()
        assert coloraxis.cmax == EXAMPLE_VALID_INT_COLOR_DATA.max()
    ...
    def test_discrete_shared_coloraxis(self):
        coloraxis = ColorSpecifier.build_from_color_data(
            color_data=EXAMPLE_VALID_INT_COLOR_DATA.astype(str), color_palette="Set3"
        ).build_coloraxis(color_data=EXAMPLE_VALID_INT_COLOR_DATA.astype(str), shared=True)
        assert coloraxis.cmin == 0
        assert coloraxis.cmax == np.max(np.arange(len(EXAMPLE_VALID_INT_COLOR_DATA.unique())))
