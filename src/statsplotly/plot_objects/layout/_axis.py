...
class BaseAxis(BaseModel):
    ...
    def validate_axis_consistency(self) -> Self:
        if self.range is not None and self.tickvals is not None:
            self.tickvals = self.range
            if self.ticktext is not None:
                try:
                    self.ticktext = [self.ticktext[idx] for idx in self.tickvals]  # type: ignore
                except TypeError:
