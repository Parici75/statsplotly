...
from statsplot.exceptions import UnsupportedColormapError
...
def rand_jitter(arr: np.ndarray, jitter_amount: float = 1) -> np.ndarray:
    """from https://stackoverflow.com/questions/8671808/matplotlib-avoiding
    -overlapping-datapoints-in-a-scatter-dot-beeswarm-plot"""
    spread = (
        0.01 * (max(arr) - min(arr)) * jitter_amount or 0.05 * jitter_amount
    )
...
def cmap_to_array(
    ...
    """Returns n_colors linearily spaced values on the colormap specified from cmap."""
...
def to_rgb(numeric_array: np.ndarray) -> List[str]:
    """Transforms a list of numeric rgb tuple into a list of rgb strings"""
    rgb_array = [
        "rgb" + str(rgb_tuple[:3])
        for rgb_tuple in list(
            map(lambda x: tuple(int(rgb * 256) for rgb in x), numeric_array)
        )
    ]
    return rgb_array
...
def get_rgb_discrete_array(
    n_colors: int, color_palette: List | str | None
) -> List[str]:
...
def compute_colorscale(
    ...
    if isinstance(color_palette, list):
        color_palette = matplotlib.colors.LinearSegmentedColormap.from_list(
            "", color_palette
        )
    ...
    if color_system is ColorSystem.LOGARITHMIC:
        ...
        for map_index, color in zip(
            np.tile(np.array([np.linspace(0, 1, n_colors + 1)]).T, 2).ravel()[
                1:-1
            ],
...
def set_rgb_alpha(color_string: str, alpha: float | None = 1) -> str:
    ...
    if re.search("rgba", color_string) is not None:
        ...
        rgba_string = (
            f"{re.sub('rgb', 'rgba', color_string)[:-1]} , {str(alpha)})"
        )
